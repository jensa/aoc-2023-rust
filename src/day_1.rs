...
fn get_digits(part: String) -> Vec<i32>{
    ...
    while i < chars.len() {
        if chars[i].is_numeric() {
            return_vec.push(chars[i].to_string().parse::<i32>().unwrap());
            i +=1;
            continue;
        }
        ...
        if i + 5 <= chars.len() {
            ...
            if words.contains_key(five_slice) {
                return_vec.push(*words.get(five_slice).unwrap());
                i+=1;
                continue;
            }
        }
        if i + 4 <= chars.len() {
            ...
            if words.contains_key(four_slice) {
                return_vec.push(*words.get(four_slice).unwrap());
                i+=1;
                continue;
            }
        }
        if i + 3 <= chars.len() {
            ...
            if words.contains_key(three_slice) {
                return_vec.push(*words.get(three_slice).unwrap());
            }
        }
        i += 1;
    }
